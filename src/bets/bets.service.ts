...
export class BetsService {
  ...
  async getPortfolio(wallet: string): Promise<{ open: Bet[]; history: Bet[] }> {
    return this.inMemoryBetsService.getPortfolio(wallet);
  }
  ...
  async placeBet(placeBetDto: {
    ...
    const market = await this.inMemoryMarketsService.findOne(
      placeBetDto.marketId,
    );

    if (!market) {
      throw new NotFoundException(
        `Market with ID ${placeBetDto.marketId} not found`,
      );
    }
    ...
    const outcomePool =
      placeBetDto.outcome === 0 ? market.pools.outcomeA : market.pools.outcomeB;
    const poolRatio =
      totalPool > 0
        ? (totalPool + placeBetDto.amount) / (outcomePool + placeBetDto.amount)
        : 2;
    ...
    await this.marketResolutionService.checkAndActivateMarket(
      placeBetDto.marketId,
    );

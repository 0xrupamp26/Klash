...
export class InMemoryBetsService {
  ...
  async getPortfolio(
    walletAddress: string,
  ): Promise<{ open: Bet[]; history: Bet[] }> {
    const userBets = Array.from(this.bets.values()).filter(
      (b) => b.walletAddress === walletAddress,
    );
    const open = userBets.filter(
      (b) =>
        b.status === 'PENDING' ||
        b.status === 'CONFIRMED' ||
        b.status === 'WAITING_FOR_SECOND',
    );
    const history = userBets.filter(
      (b) =>
  ...
  async create(betData: Partial<Bet>): Promise<Bet> {
    ...
    if (betData.transactionHash) {
      const existing = Array.from(this.bets.values()).find(
        (b) => b.transactionHash === betData.transactionHash,
      );
      if (existing) {
        console.log(
          `Idempotency check: Bet for hash ${betData.transactionHash} already exists.`,
        );
        return existing;
      }
    }
    ...
    const existingPlayer = market.currentPlayers.find(
      (p) => p.walletAddress === bet.walletAddress,
    );
    if (!existingPlayer) {
    ...
    if (market.currentPlayers.length === 1) {
      ...
      setTimeout(
        () => this.resolutionService.resolveMarket(market.marketId),
        2000,
      );
    }

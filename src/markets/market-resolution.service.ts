...
export class MarketResolutionService {
  ...
  async checkAndActivateMarket(marketId: string): Promise<void> {
    ...
    if (market.currentPlayers.length >= market.playerLimit) {
      ...
      this.logger.log(
        `Market ${marketId} activated with ${market.currentPlayers.length} players`,
      );
  ...
  async resolveMarket(marketId: string): Promise<void> {
    ...
    if (p1.outcome === p2.outcome) {
      ...
      isRefund = true;
      console.log(
        `[Resolution] Both players chose outcome ${p1.outcome}. REFUND.`,
      );
    } else {
      // Different sides -> Random 50/50
      winningOutcome = Math.random() < 0.5 ? 0 : 1;
      console.log(
        `[Resolution] Random winner selected: Outcome ${winningOutcome}`,
      );
    }
    ...
    for (const bet of bets) {
      ...
      if (winAmount > 0) {
        try {
          ...
          console.log(
            `[Payout] Sent ${winAmount} APT to ${bet.walletAddress}. Hash: ${txHash}`,
          );
        } catch (e) {
    ...
    this.marketGateway.marketResolved(marketId, {
      ...
      isRefund,
      winner:
        winningOutcome !== -1 ? market.outcomes[winningOutcome] : 'REFUND',
      totalPool,
      platformFee,
    });
  }

...
export class TwitterAiService {
  ...
  async createMarketsFromTweets(
    ...
    for (const tweet of tweets) {
      ...
      try {
        ...
        this.logger.error(
          `Failed to create market for tweet ${tweet.id}: ${error.message}`,
        );
  ...
  async runMarketCreationPipeline(): Promise<{
    ...
    try {
      ...
      const controversialTweets = tweets.filter(
        (tweet, index) => controversyResults[index]?.isControversy,
      );
      ...
      const marketResults =
        await this.createMarketsFromTweets(controversialTweets);
